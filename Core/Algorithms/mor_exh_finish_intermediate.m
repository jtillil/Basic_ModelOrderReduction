...
if mor_options.variability
    model.variability.Npop = size(mor_options.virtual_pop, 1);
    model.variability.virtual_pop = mor_options.virtual_pop;
    model.variability.X_ref_var = mor_options.X_ref_var;
    ...
    for npop = 1:model.variability.Npop
        [model.variability.redobj{npop}, ~, ~, ~] = objfun(model.t_ref, model.variability.X_ref_var{npop}, model.variability.virtual_pop(npop, :)', model.par, model.I, model.L, model.param, model.multiple, model.odefun, model.jacfun, finalconfig, mor_options.errtype);
    ...
    model.variability.q90errout = prctile(model.variability.errout(2:end), 90);
    model.variability.q90errdyn100 = prctile(model.variability.errdyn100(2:end), 90);
end
...
if ~from_repeated
    ...
    save([mor_options.saveroot '.mat'], 'redmodel', 'log', '-v7.3');
end
