...
elseif errtype == "REL"
    errfun = @(t_ref,X_ref,X_red) trapz(t_ref, abs(X_ref - X_red)./X_ref, 1) ./ (t_ref(end) - t_ref(1));
elseif errtype == "AUC"
    errfun = @(t_ref,X_ref,X_red) trapz(t_ref, abs(X_ref - X_red), 1) ./ trapz(t_ref, X_ref, 1);
else
    disp(errtype)
